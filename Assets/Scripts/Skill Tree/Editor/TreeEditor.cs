...
public class NodeBasedEditor : EditorWindow
{
    ...
    private void SaveSkillTree()
    {
        if (nodes.Count > 0)
        {
            ...
            path = "Assets/Scripts/Skill Tree/TreeData/skilltree.json";
    ...
    private void SaveNodes()
    {
        ...
        for (int i = 0; i < nodes.Count; ++i)
        {
            ...
            nodeData.nodeDataCollection[i].idNode = nodes[i].skill.idSkill;
            nodeData.nodeDataCollection[i].position = nodes[i].rect.position;
        }

        string json = JsonUtility.ToJson(nodeData);
        string path = "Assets/Scripts/Skill Tree/TreeData/nodeData.json";
        ...
        if (File.Exists(path))
        {
            ...
            if (File.Exists(path))
            {
                ...
                for (int i = 0; i < skillTree.Length; ++i)
                {
                    ...
                    {
                        if (loadedData.nodeDataCollection[j].idNode == skillTree[i].idSkill)
                        {
                            pos = loadedData.nodeDataCollection[j].position;
                            break;
                        }
                    }
                    LoadSkillCreateNode(skillTree[i], pos);
                    if (skillTree[i].skillDependencies.Length == 0)
                    {
                        originNode.Add(skillTree[i]);
                    }
                    skillDictionary.Add(skillTree[i].idSkill, skillTree[i]);
                }
                ...
                for (int i = 0; i < nodes.Count; ++i)
                {
                    for (int j = 0; j < nodes[i].skill.skillDependencies.Length; ++j)
                    {
                        if (skillDictionary.TryGetValue(nodes[i].skill.skillDependencies[j], out outSkill))
                        {
                            for (int k = 0; k < nodes.Count; ++k)
                            {
                                if (nodes[k].skill.idSkill == outSkill.idSkill)
                                {
                                    outNode = nodes[k];
    ...
    private void LoadSkillCreateNode(Skill skill, Vector2 position)
    {
        ...
        nodes.Add(new Node(position, 200, 100, nodeStyle, selectedNodeStyle,
            inPointStyle, outPointStyle, OnClickInPoint, OnClickOutPoint, OnClickRemoveNode,
            skill.idSkill, skill.unlocked, skill.cost, skill.skillDependencies, skill.description));
        ++nodeCount;
    }
}